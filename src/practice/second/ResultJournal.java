...
public class ResultJournal {
    ...
    public ResultJournal sortAcsId() {
        ResultJournal resultJournal = new ResultJournal(String.format("Журнал %s отсортированный по возрастанию id", name), new CompAscId());
        Set<Map.Entry<FilmKey, Integer>> pairs = journal.entrySet();
        for (Map.Entry<FilmKey, Integer> pair:pairs) {
    ...
    public ResultJournal sortAcsRank() {
        ResultJournal resultJournal = new ResultJournal(String.format("Журнал %s отсортированный по возрастанию места", name), new CompAscRank(journal));
        Set<Map.Entry<FilmKey, Integer>> pairs = journal.entrySet();
        for (Map.Entry<FilmKey, Integer> pair:pairs) {
            resultJournal.addFilm(pair.getKey(), pair.getValue());
        }
    ...
    public ResultJournal sortDescRank() {
        ResultJournal resultJournal = new ResultJournal(String.format("Журнал %s отсортированный по убыванию места", name), new CompDescRank(journal));
        Set<Map.Entry<FilmKey, Integer>> pairs = journal.entrySet();
        for (Map.Entry<FilmKey, Integer> pair:pairs) {
            resultJournal.addFilm(pair.getKey(), pair.getValue());
        }
